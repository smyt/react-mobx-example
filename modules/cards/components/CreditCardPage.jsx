...
import {
  Row, Col, Spin, Button, Icon, Upload, Modal, Tabs, notification, Tooltip,
} from 'antd'
...
class CreditCardPage extends Component {
  ...
  componentDidMount() {
    const me = this
    const { appStore, match } = me.props
    const { uiSingleCard, uiCategories } = appStore
  ...
  showFileOperations(record) {
    this.setState({ isGhostWindowVisible: true, file: record })
  }
  ...
  nextCard() {
    ...
    appStore.uiSingleCard.nextCard(history)
  }

  prevCard() {
    const { appStore, history } = this.props
    appStore.uiSingleCard.prevCard(history)
  }
