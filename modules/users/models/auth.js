 ...
 */
import {
  types, flow, applySnapshot, getParent, getRoot,
} from 'mobx-state-tree'
import {
  signIn, signUp, doLogout, checkStatus,
  ...
  .actions((self) => {
    ...
    const login = flow(function* login() {
      try {
        ...
        const {
          success, token, message, errors, user,
        } = yield signIn(params)
        ...
        console.error('Failed to login ', err) // eslint-disable-line
      }
    })
    ...
    const logout = flow(function* logout() {
      try {
        ...
        console.error('Failed to logout', err) // eslint-disable-line
      }
    })
    ...
    const status = flow(function* status() {
      try {
        ...
        console.error('Failed to check status ', err) // eslint-disable-line
      }
    })
