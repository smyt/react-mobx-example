  ...
  .actions((self) => {
    ...
    const save = flow(function* save(values) {
      ...
      try {
        ...
        if (success) {
          const {
            firstname, secondname, birthday, lastActive, settings,
          } = record
    ...
    const checkCategory = flow(function* checkCat(item, check) {
      const url = settingsUrl.replace(':id', self.id)

      try {
        self.isLoading = true

        const values = _.cloneDeep(self.settings)
    ...
    get excludeGraphCategories() {
      return self.settings && self.settings.excludeGraphCategories
        ? self.settings.excludeGraphCategories
        : []
    },
    get isDefaultAvatar() {
