...
import setPropertyBehavior from '../../core/models/setPropertyBehavior'
...
export const CategoriesStore = types
  ...
  .actions((self) => {
    const pRemove = self.remove
    const remove = flow(function*(data, params) {
      const url = checkCategoryPatterns.replace(':id', data.id)
      const { success, canRemove, message } = yield request({ url, method: 'POST' })
      if (success && canRemove) {
        yield pRemove(data, params)
      } else {
