...
export const AbstractStore = types.model({
  ...
  const createUrl = function (method, data) {
    ...
    if (self.restUrl) {
      ...
      switch (method) {
        case 'GET':
          url = self.url // eslint-disable-line
          break
        case 'PUT':
          url = self.createUrl
          break
  ...
  const endErrorLoading = function () {
    self.isLoaded = false
    self.isLoading = false
    self.afterError()
  }
  ...
  const load = flow(function*(reload = true, params = {}) {
    ...
    if (!self.isLoaded || reload) {
      try {
        self.isLoading = true
        const {
          success, records, errors, message,
        } = yield fetchData(url, mergedParams)
        if (success) {
  ...
  const create = flow(function*(data) {
    ...
    try {
      ...
      const {
        success, record, errors, payload,
      } = yield saveData(url, 'PUT', data)
  ...
  const removeRange = flow(function*(params) {
    ...
    try {
      ...
      const {
        removed, success, errors, payload,
      } = yield saveData(url, 'DELETE', params)
  ...
  const loadFrom = flow(function*(url, params) {
    try {
      self.isLoading = true
  ...
  const afterSuccessCreate = function () {
    if (self.useNotify) {
      getRoot(self).message('Запись успешно добавлена.')
    }
  }

  const afterSuccessUpdate = function () {
    if (self.useNotify) {
      getRoot(self).message('Запись успешно обновлена.')
    }
  }

  const afterSuccessRemove = function () {
    if (self.useNotify) {
      getRoot(self).message('Запись успешно удалена.')
    }
  }

  const afterSuccessRemoveRange = function (count) {
    if (self.useNotify) {
